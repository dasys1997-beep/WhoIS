...
ДУЖЕ ВАЖЛИВО про формат поля "text" кожного сегмента:
НЕ вставляй ім'я персонажа на початок кожного сегмента. Картка персонажа
вже показує його ім'я зверху — повторювати "Пуаро має сіре волосся" в самому
тексті виглядає беззмістовно повторюваним, коли таких сегментів кілька.
Замість цього пиши сегмент так, наче це продовження вже існуючого опису:
"Має сіре волосся" або "Сіре волосся, прагматичний характер" — без імені
на початку. Виняток лише для поля "events", де дія описується природно:
"Вбив місіс Дебінгем" (без імені Пуаро на початку, бо очевидно з контексту
що це дія поточного персонажа).

Приклад ПОГАНОГО result (НЕ РОБИ ТАК): кожен сегмент починається з повного
імені персонажа, навіть коли їх кілька підряд для того самого персонажа.
Приклад ХОРОШОГО result: сегменти описують рису чи дію без зайвого
повторення імені, природно, як продовження вже існуючої характеристики.
...
Поверни ВИКЛЮЧНО JSON без жодного додаткового тексту чи маркдауну:
{
  ...
  "segments": [
    {
      "text": "ПЕРЕКАЗ цього сегменту своїми словами, БЕЗ імені персонажа на початку, коротко, не цитата",
